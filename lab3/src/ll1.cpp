...
set<string> FIRST(const string& symbol, const Grammar& G,
                  map<string, set<string>>& firstMemo) {
    if (firstMemo.count(symbol)) return firstMemo[symbol];
    set<string> res;
        ...
        for (auto& p : G.prod) {
            ...
            for (auto& rhs : p.second) {
                for (int i = 0; i < rhs.size(); i++) {
                    ...
                    for (auto& s : fBeta)
                        if (s != "ε") follow[B].insert(s);
                    if (fBeta.find("ε") != fBeta.end() || beta.empty()) {
                        for (auto& s : follow[A]) follow[B].insert(s);
                    }
...
bool isLL1(const Grammar& G, const map<string, set<string>>& firstMemo,
    ...
    for (auto& nt : G.nonterminals) {
        ...
        for (auto& rhs : G.prod.at(nt)) {
            set<string> rhsFirst = FIRST_string(
                rhs, G, const_cast<map<string, set<string>>&>(firstMemo));
            for (auto& s : rhsFirst) {
                if (s == "ε") {
                    hasEpsilon = true;
                } else {
        ...
        if (hasEpsilon) {
            for (auto& followSym : follow.at(nt)) {
                if (allFirsts.count(followSym)) {
                    cout << "冲突：非终结符 " << nt
                         << " 的 FIRST 和 FOLLOW 重叠于 " << followSym << endl;
                    isLL1 = false;
                }
                if (parseTable.count({nt, followSym})) {
                    cout << "冲突：预测分析表中 (" << nt << ", " << followSym
                         << ") 有多个产生式" << endl;
                    isLL1 = false;
                }
...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < m; ++j) {
            ...
            if (hasSpace) {
                ...
                for (char c : trimmed) toks.push_back(string(1, c));
            }
    ...
    // 计算 FIRST 和 FOLLOW
    map<string, set<string>> firstMemo;
    for (auto& nt : G.nonterminals) FIRST(nt, G, firstMemo);
    ...
    // 收集所有终结符列（包括 $）
    set<string> columns;
    for (auto& term : G.terminals) columns.insert(term);
    columns.insert("$");

    // 准备表头
    vector<string> headers = {"非终结符  "};
    // vector<string> headers = {"NT"};
    for (auto& col : columns) headers.push_back(col);
