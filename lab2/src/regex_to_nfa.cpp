    ...
    void print() const {
        ...
        for (auto& p : states) {
            ...
            for (auto& kv : st.trans) {  // kv是map中的键值对
                char sym =
                    kv.first;  // kv.first是转换符号，kv.second是目标状态列表
...
NFA build_from_postfix(const string& postfix) {
    ...
    for (size_t i = 0; i < postfix.size(); ++i) {
        ...
        if (token == '.') {
            ...
            // '*':弹出A，并创建s和t，s ->eps-> A.start and s->eps->t;
            // A.accept->eps->A.start and ->eps->t
            Fragment A = st.top();
            st.pop();
            int s = nfa.new_state();
            int t = nfa.new_state();
...
void epsilon_closure_from(int state, unordered_set<int>& visited,
                          const NFA& nfa) {
...
unordered_set<int> epsilon_closure(const unordered_set<int>& stateset,
                                   const NFA& nfa) {
...
unordered_set<int> move_set(const unordered_set<int>& stateset, char symbol,
                            const NFA& nfa) {
    unordered_set<int> res;
    for (int s : stateset) {
